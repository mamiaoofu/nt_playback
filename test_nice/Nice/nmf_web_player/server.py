...
def nice_status() -> dict[str, object]:
    ...
    return {
        ...
        "dotnetConverter": str(TOOLS / "NiceNmfConverter.exe"),
        "dotnetReady": (TOOLS / "NiceNmfConverter.exe").exists(),
...
def convert_nmf(source: bytes, source_name: str, engine: str) -> tuple[bytes, dict[str, str]]:
    ...
    if engine == "dotnet" and not status["dotnetReady"]:
        raise RuntimeError(f".NET converter is not ready: {status['dotnetConverter']}")
...
class NmfPlayerHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            engine = query.get("engine", ["powershell"])[0]
