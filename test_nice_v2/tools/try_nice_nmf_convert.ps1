...
$method.Invoke($null, [object[]]@($InputNmf, $OutputFile, $compressionValue))

$durationSeconds = $null
try {
    $mediaMethod = $converterType.GetMethods([Reflection.BindingFlags] "Public,NonPublic,Static") |
        Where-Object { $_.Name -eq "GetNmfVoiceMedia" } |
        Select-Object -First 1
    if ($mediaMethod -ne $null) {
        $media = $mediaMethod.Invoke($null, [object[]]@($InputNmf))
        if ($media -ne $null) {
            $durationSeconds = ($media.DisplayStopTime - $media.DisplayStartTime).TotalSeconds
        }
    }
}
catch {
